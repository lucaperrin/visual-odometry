...
def process_image_pair(img1, img2, sift, bf, K, dist_coeffs, ratio_test, frame_index):
    """
    ...
    2. Match features using SIFT with Lowe's ratio test.
    3. Filter matches with BRUTE to remove outliers.
...
def compute_trajectory(image_files, directory, sift, bf, K, dist_coeffs, ratio_test):
    """
    ...
    for i in range(len(image_files) - 1):
        ...
        R, t, _, _ = process_image_pair(img1, img2, sift, bf, K, dist_coeffs, ratio_test, i + 1)
        if R is None or t is None:
...
def plot_trajectories_with_orientations(positions, orientations, smoothed_positions, ground_truth):
    ...
    # Labels and legend
    ax.set_title("SIFT-Brute: Estimated, Smoothed and Ground-Truth Trajectories")
    ax.set_xlabel("X (m)")
    ax.set_ylabel("Y (m)")
    ax.legend()
...
def main(image_directory, number_of_features, ratio_test):
    ...
    # Compute trajectory
    positions, orientations = compute_trajectory(image_files, image_directory, sift, bf, K, dist_coeffs, ratio_test)

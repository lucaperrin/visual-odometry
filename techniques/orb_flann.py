...
def load_images(directory):
    ...
    if not image_files:
        raise FileNotFoundError(f"Erreur, No image files found in directory: {directory}")
    return image_files

def filter_matches_with_ransac(pts1, pts2):
    """Filter matches using RANSAC on the fundamental matrix.
       This step removes outliers to improve pose estimation accuracy.
    """
...
def process_image_pair(img1, img2, orb, flann, K, dist_coeffs, ratio_test, frame_index):
    ...
    # Step 2: Match descriptors with FLANN
    matches = flann.knnMatch(des1, des2, k=2)

    # Step 3: Apply Lowe's ratio test to retain good matches
    good_matches = []
    for m, n in matches:
    ...
    # Step 4: Filter matches using RANSAC
    ...
    # Step 6: Visualize keypoints and movements
    ...
    # Step 7: Recover pose from the essential matrix
    _, R, t, _ = cv2.recoverPose(E, pts1, pts2, K)

    # Handle cases where pose recovery fails
...
def plot_trajectories_with_orientations(positions, orientations, smoothed_positions, ground_truth):
    ...
    # Labels and legend
    ax.set_title("Orb-Flann : Estimated, Smoothed and Ground-Truth Trajectories")
    ax.set_xlabel("X (m)")
    ax.set_ylabel("Y (m)")
    ax.legend()
...
def main(image_directory,number_of_features,ratio_test):
    ...
    # Compute trajectory
    positions, orientations = compute_trajectory(image_files, image_directory, orb, flann, K, dist_coeffs, ratio_test)

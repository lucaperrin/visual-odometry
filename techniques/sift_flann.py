...
def process_image_pair(img1, img2, sift, flann, K, dist_coeffs, ratio_test, frame_index):
    """
    Process a pair of images to estimate relative pose using feature matching.
    
    Steps:
    1. Detect and compute SIFT features for both images.
    2. Match features using FLANN with Lowe's ratio test.
...
def compute_trajectory(image_files, directory, sift, flann, K, dist_coeffs, ratio_test):
    """
    ...
    for i in range(len(image_files) - 1):
        ...
        R, t, _, _ = process_image_pair(img1, img2, sift, flann, K, dist_coeffs, ratio_test, i + 1)
        if R is None or t is None:
...
def plot_trajectories_with_orientations(positions, orientations, smoothed_positions, ground_truth):
    ...
    # Labels and legend
    ax.set_title("SIFT-FLANN: Estimated, Smoothed and Ground-Truth Trajectories")
    ax.set_xlabel("X (m)")
    ax.set_ylabel("Y (m)")
    ax.legend()
...
def main(image_directory, number_of_features, ratio_test):
    ...
    # Initialize SIFT
    sift = cv2.SIFT_create(nfeatures=number_of_features)

    # Initialize FLANN Matcher
    index_params = dict(algorithm=1, trees=5)
    search_params = dict(checks=50)  # Increase the number of checks for better accuracy
    ...
    # Compute trajectory
    positions, orientations = compute_trajectory(image_files, image_directory, sift, flann, K, dist_coeffs, ratio_test)
